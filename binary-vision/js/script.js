...
var $page = $('html, body');
$('.see-positions-block a[href*="#"]').click(function (e) {
    e.preventDefault();
    $page.animate({
...
$(".describe").on('input', function () {
    ...
    if (text !== '') {
        ...
        if (document.querySelectorAll(".write-value").length < 3) {
...
if ( $("body").hasClass("home") ) {
    (function(){
        $(window).ready(function(){
            $("#s-projects-slider, #s-clients-slider").mThumbnailScroller({
              axis:"x"
          });
        });
    })(); 
...
$(".form-connect").submit( function(event) {
    ...
    var formData = new FormData(this);
    formData.append('service_id', 'hello_binary_vision');
    formData.append('template_id', 'template_HsKrfsH2_connect');
    formData.append('user_id', 'user_zkugnNx2fhfZS4gHt7A02');
    ...
    $.ajax('https://api.emailjs.com/', {
        ...
        $(".form-connect")[0].reset();
        $(".connect-btn-wrap .btn-hov").addClass("btn-hov-hide-b");
        setTimeout ( function() {
            $(".connect-success-f").addClass("connect-success-vis");
        }, 1000);
        setTimeout ( function() {
            $(".connect-btn-wrap .btn-hov").removeClass("btn-hov-hide-b");
            $(".connect-success-f").removeClass("connect-success-vis");
        }, 4000);
    }).fail(function(error) {
        alert('Oops... ' + JSON.stringify(error));
    });
